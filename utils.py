...
from info import AUTH_CHANNEL, REQUEST_TO_JOIN_MODE, LONG_IMDB_DESCRIPTION
...
async def is_subscribed(bot, query, channel):
    ...
    for id in channel:
        ...
        if REQUEST_TO_JOIN_MODE == True and join_db().isActive():
            user = await join_db().get_user(query.from_user.id, int(id))
            if user:
                continue
            else:
                try:
                    ...
                    link = await bot.create_chat_invite_link(chat_id=int(id), creates_join_request=True)
                    btn.append([InlineKeyboardButton(f'Join {title}', url=link.invite_link)])
            ...
            try:
                ...
                link = await bot.create_chat_invite_link(chat_id=int(id))
                btn.append([InlineKeyboardButton(f'Join {title}', url=link.invite_link)])
            except Exception as e:
                pass
    return btn
